...
class BaseModel(nn.Module):
    def __init__(self,cfgs,training):
        ...
        self.direction_estimation = DirectionEstimation()
        self.direction_estimation.build_network(cfgs['model_cfg'])
        self.direction_estimation.init_parameters()
        ...
        if training:
            ...
            self.direction_optimizer = self.get_optimizer(self.cfgs['optimizer_cfg'])
            # self.direction_loss = LossAggregator(cfgs['loss_cfg'])
            self.direction_scheduler = self.get_scheduler(cfgs['scheduler_cfg'],self.direction_optimizer)
        self.train(training)
    ...
    def run_train(model):
        ...
        for inputs in model.train_loader:
            ...
            with autocast(enabled=model.engine_cfg['enable_float16']):
                retval = model(ipts)
                training_feat, visual_summary, features = retval['training_feat'], retval['visual_summary'], retval ['extracted_features']
                direction_retval = model.direction_estimation(features,ipts[1],ipts[2],ipts[3],ipts[4])
                direction_training_feat = retval['training_feat']
                del direction_retval
                del retval
            direction_loss = model.loss_aggregator(direction_training_feat)
            loss_sum, loss_info = model.loss_aggregator(training_feat)
            total_loss = loss_sum - direction_loss
            ok = model.train_step(total_loss)
            direction_ok = model.direction_estimation(direction_loss)
            if not ok or not direction_ok:
                continue
