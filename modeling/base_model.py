...
class BaseModel(nn.Module):
    ...
    def _load_ckpt(self, save_name):
        ...
        map_location = f"cuda:{self.device}" if torch.cuda.is_available() else "cpu"
    ...
    def inputs_pretreament(self, inputs):
        ...
        # print(vies)
        # print(labs_batch)
        # # Create a mapping for vies_batch
        # unique_vies = list(set(vies_batch))
        # vies_mapping = {v: i for i, v in enumerate(unique_vies)}
        # vies = [vies_mapping[v] for v in vies_batch]
        vies = list2var(vies).long()
    ...
    def run_train(model):
        ...
        for inputs in model.train_loader:
            ...
            # print(np.shape(training_feat[triplet['embeddings']].shape))
    ...
    def run_test(model):
        ...
        rank = 0
        with torch.no_grad():
            info_dict = model.inference(rank=0)
        if rank == 0:
