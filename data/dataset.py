...
class DataSet(tordata.Dataset):
    ...
    def __dataset_parser(self, data_config, training):
        ...
        with open(data_config['dataset_partition'], "rb") as f:
            partition = json.load(f)
        train_views = partition["TRAIN_SET"]
        test_views = partition["TEST_SET"]

        label_list = os.listdir(dataset_root)
        msg_mgr = get_msg_mgr()

        def log_view_list(view_list):
            if len(view_list) >= 3:
                msg_mgr.log_info('[%s, %s, ..., %s]' %
                                 (view_list[0], view_list[1], view_list[-1]))
            else:
                msg_mgr.log_info(view_list)

        if training:
            msg_mgr.log_info("-------- Train View List --------")
            log_view_list(train_views)
        else:
            msg_mgr.log_info("-------- Test View List --------")
            log_view_list(test_views)

        def get_seqs_info_list(view_set):
            seqs_info_list = []
        ...
        self.seqs_info = get_seqs_info_list(train_views) if training else get_seqs_info_list(test_views)
